import { initDB, run, saveDB } from './client';
...
async function seed() {
  ...
  run('DELETE FROM lead_custom_field_values');
  run('DELETE FROM custom_fields');
  ...
  run(`INSERT INTO custom_fields (id, tenant_id, label, type, status) VALUES (?, ?, ?, ?, ?)`, [CITY_FIELD, TENANT_A, 'City', 'string', 1]);
  run(`INSERT INTO custom_fields (id, tenant_id, label, type, status) VALUES (?, ?, ?, ?, ?)`, [SCORE_FIELD, TENANT_A, 'Score', 'number', 1]);
  run(`INSERT INTO custom_fields (id, tenant_id, label, type, status) VALUES (?, ?, ?, ?, ?)`, [uuidv4(), TENANT_B, 'City', 'string', 1]);

  for (const lead of tenantALeads) {
    run(`INSERT INTO lead_custom_field_values (id, lead_id, field_id, value) VALUES (?, ?, ?, ?)`, [uuidv4(), lead.id, CITY_FIELD, lead.city]);
    run(`INSERT INTO lead_custom_field_values (id, lead_id, field_id, value) VALUES (?, ?, ?, ?)`, [uuidv4(), lead.id, SCORE_FIELD, lead.score]);
  }
  ...
  console.log('City Custom Field ID:', CITY_FIELD);
  console.log('Score Custom Field ID:', SCORE_FIELD);
  console.log('---');
  console.log('Tenant A leads:', tenantALeads.length);
  console.log('Tenant B leads:', tenantBLeads.length);
}
